...
static int ov5675_write_reg(const struct device *dev, uint16_t addr, uint8_t value)
{
...
static int ov5675_write_all(const struct device *dev, const struct ov5675_reg *regs, size_t num)
{
	for (size_t i = 0; i < num; i++) {
		int err = ov5675_write_reg(dev, regs[i].addr, regs[i].value);
...
static int ov5675_hard_reseten(const struct device *dev)
{
	const struct ov5675_config *cfg = dev->config;
	int ret;
...
static int ov5675_init(const struct device *dev)
{
	...
	ret = ov5675_write_reg(dev, OV5675_REG_MODE_SELECT, OV5675_MODE_STANDBY);
	if (ret) {
...
static int ov5675_set_stream(const struct device *dev, bool enable)
{
	...
	ret = ov5675_write_reg(dev, OV5675_REG_MODE_SELECT, val);
	if (ret) {
